...
def train(modelfile, inputfolder, maskfolder, checkpointfolder, epochs, steps_per_epoch):
    ...
    # X_train_p, y_train_p = prepare_data(
    #     X_train, y_train, patch_size)
    # X_train_p = preprocess_input(X_train_p)
    # y_train_p = tf.cast(y_train_p, tf.float32)
    ...
    for epoch in range(epochs):
        ...
        for i in range(int(steps_per_epoch)):
            sampled_data_c, sampled_labels_c = random_block_sampling(X_train, y_train, block_size, 2000)
        ...
        # predictions = model.predict(X_train_c)
        # print(predictions.shape)
        # test_loss = total_loss(y_train_c, predictions).numpy()
        # print("Test loss:", test_loss)

        # iou_score = sm.metrics.IOUScore(threshold=0.5)
        # test_iou_score = iou_score(y_train_c, predictions).numpy()
        # print("Test IOU Score:", test_iou_score)

        # f1_score = sm.metrics.FScore(threshold=0.5)
        # test_f1_score = f1_score(y_train_c, predictions).numpy()
        ...
        if epoch != 0 and (epoch+1) % 50 == 0:
            model.save(modelfile + "random_block_sampling_v3_augmentations" + str(epoch+1) + ".h5")
...
def main():
    ...
    modelfile = "/home/chongyuh/3dunet/model/random_block_sampling/"
