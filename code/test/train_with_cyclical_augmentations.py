...
def plot_loss_and_iou(history, currenttime):
    ...
    plt.tight_layout()
    plt.savefig('train_with_cyclical_augmentations_result_100.png')
...
def train(modelfile, inputfolder, maskfolder, checkpointfolder, epochs, start_epochs, load_model = None):
    ...
    for epoch in range(epochs//25):
        print("Epoch:",epoch*25+1+start_epochs)
        
        history = None

        X_train_c, y_train_c = prepare_data(X_train_a, y_train_a, patch_size)
        X_train_c = preprocess_input(X_train_c)
        y_train_c = tf.cast(y_train_c, tf.float32)
        history = model.fit(X_train_c, y_train_c,validation_data=(X_test, y_test), batch_size=1, epochs=25,verbose=1)
        ...
        if epoch != 0 and (epoch*25+1+start_epochs) % 100 == 0:
            model.save(modelfile + "train_with_cyclical_augmentations" + str(epoch*25+1+start_epochs) + ".h5")
...
def main():
    ...
    modelfile = "/home/chongyuh/3dunet/model/"
    ...
    checkpointfolder = "/mnt/research-data/checkpoint_test/random_block_sampling_2_augmentations/"
    load_model = "None"
    start_epochs = 0
    epochs = 100

    train(modelfile, inputfolder, maskfolder, checkpointfolder, epochs, start_epochs, load_model = load_model)
