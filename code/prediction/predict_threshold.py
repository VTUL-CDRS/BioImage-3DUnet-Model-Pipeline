    ...
    for i in range(patches.shape[0]):
        for j in range(patches.shape[1]):
            for k in range(patches.shape[2]):
                input_img = patches[i, j, k, :, :, :]
                # single_patch_3ch = np.stack((single_patch,)*3, axis=-1)

                mask_A = np.logical_and(input_img >= 0, input_img < 90)
                masked_input_img_A = np.where(mask_A, input_img, 0)

                # For pixel values between 90 and 150 (inclusive)
                mask_B = np.logical_and(input_img >= 90, input_img <= 150)
                masked_input_img_B = np.where(mask_B, input_img, 0)

                # For pixel values between 151 and 255 (inclusive)
                mask_C = np.logical_and(input_img > 150, input_img <= 255)
                masked_input_img_C = np.where(mask_C, input_img, 0)

                masked_input_img_D = masked_input_img_A + masked_input_img_C

                single_patch_3ch = np.stack((masked_input_img_D, masked_input_img_B, input_img), axis=-1)
