...
def get_augmentation_all(patch_size):
    return Compose([
        ...
        Flip(axis=1, p=0.3),
        Flip(axis=2, p=0.3),
        GaussianNoise(var_limit=(0, 0.01), p=0.3),
...
def plot_loss_and_iou(history, currenttime):
    ...
    plt.ylabel('Loss')
    plt.legend()
    plt.savefig(currenttime+'threshold_augmentations_loss_9.png')
    ...
    plt.ylabel('IOU')
    plt.legend()
    plt.savefig(currenttime+'threshold_augmentations_iou_9.png')
    ...
    plt.ylabel('f1_score')
    plt.legend()
    plt.savefig(currenttime+'threshold-f1_score_augmentations_9.png')
...
def prepare_data(imagelist, masklist, patch_size=64, test_size=0.1):
    ...
    # For pixel values between 0 and 70 (inclusive)
    mask_A = np.logical_and(input_img >= 0, input_img < 70)

    # For pixel values between 70 and 180 (inclusive)
    mask_B = np.logical_and(input_img >= 70, input_img <= 180)

    # For pixel values between 180 and 255 (inclusive)
    mask_C = np.logical_and(input_img > 180, input_img <= 255)

    masked_input_img_A = np.where(mask_A, input_img,0) + np.where(mask_B, input_img,0) + np.where(mask_C, 255,0)
    masked_input_img_B = np.where(mask_A, 1,0) + np.where(mask_B, input_img,0) + np.where(mask_C, 255,0)
    masked_input_img_C = np.where(mask_A, 1,0) + np.where(mask_B, input_img,0) + np.where(mask_C, input_img,0)

    train_img = np.stack((masked_input_img_A, masked_input_img_B, masked_input_img_C), axis=-1)
...
def train(modelfile, inputfolder, maskfolder, checkpointfolder, iterations):
    ...
    X_train, y_train = generate_augmented_data(X_train, y_train, num_augmentations=5)
...
def main():
    ...
    modelfile = "/home/chongyuh/3dunet/model/threshold_augmentations_9.h5"
    inputfolder = "/home/chongyuh/3dunet/data/images/"
    maskfolder = "/home/chongyuh/3dunet/data/masks/"
    checkpointfolder = "/mnt/research-data/checkpoint_test/augmentations_threshold_9/"
    epochs = 100
    print("start with")
