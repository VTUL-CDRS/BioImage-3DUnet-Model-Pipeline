...
def generate_augmented_data(train_images, mask_images, num_augmentations):
    ...
    for i in tqdm(range(len(train_images))):
        ...
        shared = { 'data': {'image': img, 'mask': mask}, 'aug': aug}
        
        with WorkerPool(n_jobs=num_augmentations, shared_objects=shared) as pool:
            results = pool.map(augment, range(num_augmentations))
        
        for j in range(num_augmentations):
            imagelist.append(results[j][0])
            masklist.append(results[j][1])
    ...
    while (early_stop != early_stop_number) and (recovery_stops != early_stop_number):
        ...
        if (epoch) % check_epoch == 0:
            ...
            if average_val_iou > best_val_iou:
                print(f'Save best pt at epoch: {epoch}, best_val_iou: {average_val_iou}')
                best_val_iou = average_val_iou
                model.save(modelfile + "_best.h5")
                if epoch > 50:
...
def main(config: str = "trainning_config.txt"):
