...
def augment_single_image(img, mask, num_augmentations):
    imagelist = [img]
    masklist = [mask]
    ...
    for _ in range(num_augmentations):
        ...
        aug_data = aug(**data)
        imagelist.append(aug_data['image'])
        masklist.append(aug_data['mask'])
...
def generate_augmented_data(train_images, mask_images, num_augmentations):
    ...
    with concurrent.futures.ThreadPoolExecutor() as executor:
        futures = [executor.submit(augment_single_image, train_images[i], mask_images[i], num_augmentations) 
                   for i in range(len(train_images))]

        for future in concurrent.futures.as_completed(futures):
            imgs, masks = future.result()
            imagelist.extend(imgs)
            masklist.extend(masks)
...
def train(modelfile, inputfolder, maskfolder, steps_per_epoch, train_ratio, batch_size, learning_rate, cyclical_augmentations, num_augmentations, random_block_sampling_pixel, early_stop_number, augmentation_threshold_f, augmentation_threshold_iou, weights = None, load_model = None, val_inputfolder = None, val_maskfolder = None):
...
#     if val_inputfolder == None and val_maskfolder == None:
...
#     y_test = tf.cast(y_test, tf.float32)
    ...
    y_train = []
    if val_inputfolder == "None" and val_maskfolder == "None":
        print("No validation folders provided, splitting the data for training and testing.")
        X_train, X_test, y_train, y_test = process_all_data(train_images, mask_images, train_ratio)
    else:
        print("Validation folders provided, using them for X_test and y_test.")
        X_test = read_input_images(val_inputfolder)
        y_test = read_input_images(val_maskfolder)
        ...
        for i, j in zip(X_test, y_test):
            print(f"Processing validation data pair {index}:")
            print("val_raw_shape: ", i.shape)
            print("val_mask_shape: ", j.shape)
            assert i.shape == j.shape, f"Shape mismatch at index {index}: {i.shape} vs {j.shape}"
            index += 1
                ...
                if recovery_augmentations:
                    ...
                    if val_f1_score_increase < augmentation_threshold_f:
                        ...
                        if (val_f1_score_increase < 0):
                            ...
                            recovery_iou = second_last_f1_score
                            recovery_f1 = second_last_val_iou_score
...
def main():
    ...
    val_inputfolder = parameters["val_inputfolder"]  
    val_maskfolder = parameters["val_maskfolder"]  
    ...
    print(f"val_inputfolder: {val_inputfolder}")
    print(f"val_maskfolder: {val_maskfolder}")
    print(f"load_model: {load_model}")
    print(f"steps_per_epoch: {steps_per_epoch}")

    batch_size = 1
    learning_rate = 0.0001
    cyclical_augmentations = 3
    train_ratio = 0.8
    num_augmentations = 5
    early_stop_number = 10
    ...
    train(modelfile, inputfolder, maskfolder, steps_per_epoch, train_ratio, batch_size, learning_rate, cyclical_augmentations, num_augmentations, random_block_sampling_pixel, early_stop_number, augmentation_threshold_f, augmentation_threshold_iou, weights = weights, load_model = load_model, val_inputfolder = val_inputfolder, val_maskfolder = val_maskfolder)
